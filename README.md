...
- 使用无第三方依赖的 Python 脚本审计数量、比例、必填字段、引用键、DOI 和出版信息。
...
```text
literature-search/scripts/audit_literature_bib.py
```

请先为项目创建独立虚拟环境。Windows PowerShell 示例：

```powershell
python -m venv .venv
.\.venv\Scripts\python.exe literature-search\scripts\audit_literature_bib.py `
  literature_references.bib `
  --report literature_audit.txt
```
...
.\.venv\Scripts\python.exe literature-search\scripts\audit_literature_bib.py `
  literature_references.bib `
  --report literature_audit.txt `
  --json-report literature_audit.json `
  --target-count 150 `
  --chinese-ratio 0.40 `
  --recent-ratio 0.50
```

脚本仅使用 Python 标准库。退出码 `0` 表示通过，`1` 表示审计发现问题，`2` 表示文件或 BibTeX 解析错误。审计失败时，应修正文献数据后重新运行，不得使用未核验或虚假文献补足数量。
